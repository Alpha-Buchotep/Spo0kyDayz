...
function spookyInit(){
	...
	spookyMinIdo = spookyMinIdo*1000;
	spookyMaxIdo = spookyMaxIdo*1000;
...
function autoSpooky() {
	...
	addDiv();
	spkRmvTmr = setTimeout(spookyRemove, 35000, "timeout");	
...
function addDiv() {
   ...
   // Megjelenő Spo0ky pixelben megadott minimális pozíciója (x és y)
    ...
    let spkySzovegNum = Math.floor(Math.random() * (maxNum - minNum) + minNum);
...
function spookyClicked() {
	...
	spkTxtTmr = setTimeout(spookyTxtClicked, 15000);
}
...
function spookyTxtClicked () {
	...
	spkRmvTmr = setTimeout(spookyRemove, 1500, "clicked");
